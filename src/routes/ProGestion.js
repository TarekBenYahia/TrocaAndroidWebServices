...
app.post('/inscriptionPro/',(req,res,next)=>{
    ...
    mysqlConnection.query('SELECT * FROM `professionnel` where emailPro=?',[email],function (err,result,fields) {
        ...
        if (result && result.length)
            ...
            mysqlConnection.query('INSERT INTO `professionnel` (`NomPrenomPro`, `emailPro`, `encrypted_password`,`salt`, `actif`, `emailToken`, `telPro`, `dateNaissPro`, `numCinPro`, `CinPro`, `adressePro`, `nbAnneeExp`, `CartePro`, `idCategorie`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[name,email,password,salt,actif,emailToken,tel,dateN,numCIN,null,adresse,nbAnnee,cartePro,idC],function (err,result,fields) {
                mysqlConnection.on('error',function (err) {
                    console.log('[MySQL ERROR]',err);
                ...
                else res.json('enregistré avec succès');
                //mail
                var mailOptions={
...
app.put('/NoterPr/:idPro',(req,res,next)=> {
    var post_data = req.body;
    var note = parseInt(post_data.note,10);


    mysqlConnection.query('select * from professionnel where idPro=?', [req.params.idPro], function (error, result, fields) {
        var nbr = result[0].nbNote + 1;
       var noteGlobale = parseFloat(((result[0].notePro*result[0].nbNote) + note)/ nbr) ;
        //var noteGlobale = 5 ;

        if (error){console.log(error)}
        else {
            noteGl=noteGlobale.toFixed(2);
            mysqlConnection.query('UPDATE `professionnel` SET `notePro`=?,`nbNote`=?  WHERE `idPro`=?', [noteGl, nbr, req.params.idPro], function (err, result, fields) {
...
app.put('/NoterCommande/:idCommande',(req,res,next)=> {
    var post_data = req.body;
    var note = parseInt(post_data.note,10);

            mysqlConnection.query('UPDATE `commandespayes` SET `note`=?  WHERE `idCommande`=?', [note, req.params.idCommande], function (err, result, fields) {
                res.json('Note attribuée');
            });
})

app.get('/NomPro/:idPro',(request,response)=>{
    mysqlConnection.query('select emailPro from professionnel where idPro = ?',[request.params.idPro],(err,rows,fields)=>{
        if(!err)
            response.end(JSON.stringify(rows[0]));
        else console.log(err)
    })
});
app.get('/CmdPayees/:idPro',(request,response)=>{
    mysqlConnection.query('select * from commandespayes where idPro = ?',[request.params.idPro],(err,rows,fields)=>{
        if(!err)
