...
app.get("/pass",(req,res,next)=>{
    ...
    console.log('encrypt: ' + saltHashPassword("6666s").passwordHash);
    console.log('salt: ' + saltHashPassword("215").salt);
...
app.post('/verifyEmail/:emailToken',(request,response)=>{
    mysqlConnection.query('UPDATE `client` SET `actif`=?,`emailToken`=?  WHERE `emailToken`=?',[actif,null,request.params.emailToken],function (err,rows,fields){
...
app.post('/ForgotPass/',(request,response)=>{
    ...
    mysqlConnection.query('select * from client where emailClient=?', [mailC], function (error, result, fields) {
        ...
        if (result && result.length) {
            ...
            mysqlConnection.query('UPDATE `client` SET `actif`=?,`encrypted_password`=?  WHERE `emailClient`=?',[false,token,mailC],function (err,rows,fields){
                if (err)console.log(err);
                else console.log('done');
                // res.redirect('/login/');
            })
