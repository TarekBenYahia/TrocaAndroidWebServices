...
//Récupérer un employé selon son id

app.get('/tests/:id',(request,response)=>{
    mysqlConnection.query('select * from test where id = ?',[request.params.id],(err,rows,fields)=>{
        if(!err)
            response.send(rows);
        else console.log(err)
    })
...
app.post('/inscription/',(req,res,next)=>{
   ...
   // console.log(emailToken);
    //var id= post_data.id;
    //console.log(name);
    mysqlConnection.query('SELECT * FROM `test` where email=?',[email],function (err,result,fields) {
        ...
        if (result && result.length)
            ...
            mysqlConnection.query('INSERT INTO `test` (`email`, `name`, `encrypted_password`, `salt`, `actif`, `emailToken`) VALUES (?,?,?,?,?,?)',[email,name,password,salt,0,emailToken],function (err,result,fields) {
                mysqlConnection.on('error',function (err) {
                    console.log('[MySQL ERROR]',err);
...
app.post('/login/',(req,res,next)=> {
    ...
    mysqlConnection.query('select * from test where email=?', [email], function (error, result, fields) {
        mysqlConnection.on('error', function (err) {
            console.log('[MySQL ERROR]', err);
            res.json('register error', err);
            // console.log(email);
...
app.post('/verifyEmail/:emailToken',(request,response)=>{
    mysqlConnection.query('UPDATE `test` SET `actif`=?,`emailToken`=?  WHERE `emailToken`=?',[actif,null,request.params.emailToken],function (err,rows,fields){
